/**
 * Tips Data - Initial tip content for the Rucio WebUI
 * Tips are organized by category and include contextual help for key features
 */

import { Tip, TipCategory } from './tip-registry';

export const TIPS: Tip[] = [
    // ============================================
    // GETTING STARTED
    // ============================================
    {
        id: 'welcome-rucio',
        title: 'Welcome to Rucio WebUI',
        content:
            'Rucio is a scientific data management system. Use this interface to browse, search, and manage your data across distributed storage.',
        category: TipCategory.GETTING_STARTED,
        priority: 'essential',
        pages: ['/dashboard'],
        showOnFirstVisit: true,
        variant: 'info',
        learnMoreUrl: 'https://rucio.github.io/documentation/',
    },
    {
        id: 'command-palette',
        title: 'Quick Navigation',
        content: 'Press Cmd/Ctrl + K anywhere to open the command palette for quick navigation and search.',
        category: TipCategory.NAVIGATION,
        priority: 'essential',
        showOnFirstVisit: true,
        variant: 'info',
    },

    // ============================================
    // DATA IDENTIFIERS (DIDs)
    // ============================================
    {
        id: 'did-search-scope',
        title: 'Searching for DIDs',
        content:
            'Use the scope:name format to search for DIDs. For example, "mc16_13TeV:data*" finds everything matching "data*" in that scope. Wildcards (*) are supported in the name.',
        category: TipCategory.DIDS,
        priority: 'essential',
        pages: ['/dids'],
        showOnFirstVisit: true,
        variant: 'info',
    },
    {
        id: 'did-types',
        title: 'DID Types',
        content:
            'DIDs come in three types: Files (individual data files), Datasets (collections of files), and Containers (collections of datasets or other containers).',
        category: TipCategory.DIDS,
        priority: 'essential',
        pages: ['/dids', '/did'],
        variant: 'info',
    },
    {
        id: 'did-search-types',
        title: 'Search Types',
        content:
            'All is the default. It looks for containers first, then datasets, then files, and shows you the first kind it finds, with the type of each result in the table. Picking Container, Dataset or File searches only that type. All cannot search files by wildcard, so refine the name if it asks you to.',
        category: TipCategory.DIDS,
        priority: 'essential',
        pages: ['/dids'],
        variant: 'info',
    },
    {
        id: 'did-metadata',
        title: 'DID Metadata',
        content: 'Click on any DID to view its metadata, including replicas, rules, and parent/child relationships.',
        category: TipCategory.DIDS,
        priority: 'helpful',
        pages: ['/dids'],
        variant: 'info',
    },
    {
        id: 'did-replicas',
        title: 'File Replicas',
        content: 'Files can have multiple replicas across different storage elements (RSEs). The "Replicas" tab shows where each copy is stored.',
        category: TipCategory.DIDS,
        priority: 'helpful',
        pages: ['/did'],
        variant: 'info',
    },

    // ============================================
    // RULES
    // ============================================
    {
        id: 'rules-overview',
        title: 'What are Rules?',
        content:
            'Rules define where data should be stored. A rule requests copies of DIDs at specific RSEs. Rucio automatically manages data placement based on rules.',
        category: TipCategory.RULES,
        priority: 'essential',
        pages: ['/rules', '/rule/create'],
        showOnFirstVisit: true,
        variant: 'info',
    },
    {
        id: 'rule-states',
        title: 'Rule States',
        content:
            'Rules progress through states: REPLICATING (copying data), OK (all copies made), STUCK (transfer issues), or SUSPENDED (paused). Check the state to monitor progress.',
        category: TipCategory.RULES,
        priority: 'essential',
        pages: ['/rules', '/rule'],
        variant: 'info',
    },
    {
        id: 'rule-lifetime',
        title: 'Rule Lifetime',
        content: 'Rules can have an expiration date. When a rule expires, Rucio may delete the associated replicas if no other rules protect them.',
        category: TipCategory.RULES,
        priority: 'helpful',
        pages: ['/rule/create', '/rule'],
        variant: 'warning',
    },
    {
        id: 'create-rule',
        title: 'Creating a Rule',
        content:
            'To create a rule, select the DIDs you want to replicate, choose the destination RSE(s), specify the number of copies, and optionally set a lifetime.',
        category: TipCategory.RULES,
        priority: 'helpful',
        pages: ['/rule/create'],
        variant: 'info',
    },

    // ============================================
    // RSEs (Rucio Storage Elements)
    // ============================================
    {
        id: 'rse-overview',
        title: 'What are RSEs?',
        content:
            'RSEs (Rucio Storage Elements) represent physical or logical storage locations where your data can be stored. Each RSE has specific protocols and quotas.',
        category: TipCategory.RSES,
        priority: 'essential',
        pages: ['/rses'],
        showOnFirstVisit: true,
        variant: 'info',
        learnMoreUrl: 'https://rucio.github.io/documentation/started/concepts/rse_expressions',
    },
    {
        id: 'rse-attributes',
        title: 'RSE Attributes',
        content:
            'RSEs have attributes like availability (for read/write/delete), supported protocols, and quota limits. Check these when choosing where to store data.',
        category: TipCategory.RSES,
        priority: 'helpful',
        pages: ['/rses', '/rse'],
        variant: 'info',
    },
    {
        id: 'rse-protocols',
        title: 'Storage Protocols',
        content: 'RSEs support different access protocols (e.g., root://, davs://, gsiftp://) that can be used to read/write data.',
        category: TipCategory.RSES,
        priority: 'advanced',
        pages: ['/rse'],
        variant: 'info',
    },

    // ============================================
    // SUBSCRIPTIONS
    // ============================================
    {
        id: 'subscriptions-overview',
        title: 'What are Subscriptions?',
        content:
            'Subscriptions automatically create rules for new data matching specific patterns. They help automate data placement as new files are registered.',
        category: TipCategory.SUBSCRIPTIONS,
        priority: 'essential',
        pages: ['/subscriptions'],
        showOnFirstVisit: true,
        variant: 'info',
        learnMoreUrl: 'https://rucio.github.io/documentation/started/concepts/subscriptions',
    },
    {
        id: 'subscription-filters',
        title: 'Subscription Filters',
        content: 'Subscriptions use filters to match DIDs. Filters can include scope patterns, DID name patterns, and metadata conditions.',
        category: TipCategory.SUBSCRIPTIONS,
        priority: 'helpful',
        pages: ['/subscriptions', '/subscription'],
        variant: 'info',
    },

    // ============================================
    // NAVIGATION & ADVANCED
    // ============================================
    {
        id: 'keyboard-shortcuts',
        title: 'Keyboard Shortcuts',
        content: 'Use Cmd/Ctrl + K for command palette. Press Escape to close dialogs and panels.',
        category: TipCategory.NAVIGATION,
        priority: 'helpful',
        variant: 'info',
    },
    {
        id: 'bookmarks',
        title: 'Bookmark Pages',
        content: 'Bookmark frequently accessed DIDs, rules, or RSEs in dashboard for quick access.',
        category: TipCategory.NAVIGATION,
        priority: 'helpful',
        variant: 'info',
    },
    {
        id: 'dark-mode',
        title: 'Dark Mode',
        content: 'The interface automatically adapts to your system theme preferences. Toggle dark mode in your OS settings.',
        category: TipCategory.ADVANCED,
        priority: 'advanced',
        variant: 'info',
    },
];

/**
 * Get all tips
 */
export function getAllTips(): Tip[] {
    return TIPS;
}

/**
 * Get the number of tips
 */
export function getTipCount(): number {
    return TIPS.length;
}
